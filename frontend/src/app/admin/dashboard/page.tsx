...
import { Users, Shield, UserPlus, LogOut, AlertCircle, CheckCircle2, Bot, Lock, Sun, Moon, Heart, ChevronDown, ChevronUp } from "lucide-react";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        const res = await fetch(`${API_BASE_URL.replace('/api', '')}/api/admin/users`, {
          headers: {
  ...
  const handleCreateAdmin = async (e: React.FormEvent) => {
    ...
    try {
      const token = localStorage.getItem("finance_token");
      const res = await fetch(`${API_BASE_URL.replace('/api', '')}/api/admin/users/admin`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        throw new Error("Error al crear administrador. Quizás el usuario ya existe.");
      }

      setFormSuccess(`Administrador "${newAdminUser}" creado con éxito.`);
      setNewAdminUser("");
      setNewAdminPass("");
      
      const resUsers = await fetch(`${API_BASE_URL.replace('/api', '')}/api/admin/users`, {
  ...
  if (loading) {
    return <div className="h-screen w-screen bg-[#0B2545] text-[#8DA9C4] flex items-center justify-center">Cargando Panel de Administración...</div>;
  }
      ...
      <main className="w-full max-w-7xl mx-auto flex-1 min-h-0 bg-[var(--brand-card)] rounded-b-2xl p-4 md:p-5 grid grid-cols-1 lg:grid-cols-3 gap-5 shadow-2xl border border-[var(--brand-border)] border-t-0 overflow-hidden">
        ...
        <div className="lg:col-span-2 flex flex-col gap-2 min-h-0 h-full">
          <div className="flex items-center gap-2 flex-shrink-0 mb-1">
            ...
            <h3 className="text-[var(--brand-muted)] font-bold tracking-widest text-[11px] uppercase">Base de Datos de Usuarios</h3>
          </div>
            ...
            {error ? (
              ...
              <div className="overflow-auto flex-1 pr-1 relative">
                <table className="w-full text-left border-collapse relative">
                  ...
                  <tbody>
                    {users.map((user) => (
                      <tr key={user.id} className="border-b border-[var(--brand-border)] hover:bg-[var(--brand-card)]/30 transition-colors">
                        <td className="py-3 px-3 text-xs text-[var(--brand-accent)] font-mono">#{user.id}</td>
                        <td className="py-3 px-3 text-sm font-semibold">{user.username}</td>
        ...
        <div className="flex flex-col gap-2 lg:pl-5 lg:border-l border-[var(--brand-border)] overflow-y-auto pr-1">
          <div className="flex items-center gap-2 flex-shrink-0 mb-1">
            ...
            <h3 className="text-[var(--brand-muted)] font-bold tracking-widest text-[11px] uppercase">Añadir Administrador</h3>
          </div>
          ...
          <div className="bg-[var(--brand-bg)] border border-[var(--brand-border)] rounded-xl p-4 flex flex-col gap-4">
            <p className="text-[11px] text-[var(--brand-muted)] leading-snug">
              Utiliza este formulario exclusivo para conceder acceso total de <strong className="text-[var(--brand-accent)]">Administrador</strong> a nuevos miembros del equipo.
            </p>

            {formError && (
              <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-lg flex items-start gap-2">
                <AlertCircle className="w-4 h-4 text-red-400 flex-shrink-0 mt-0.5" />
                <p className="text-red-400 text-xs leading-snug">{formError}</p>
              </div>
            )}
            ...
            {formSuccess && (
              <div className="p-3 bg-[var(--brand-accent)]/10 border border-[var(--brand-accent)]/30 rounded-lg flex items-start gap-2">
                ...
                <p className="text-[var(--brand-accent)] text-xs leading-snug">{formSuccess}</p>
              </div>
            )}

            <form onSubmit={handleCreateAdmin} className="space-y-3">
              <div>
                <label className="block text-[11px] font-semibold text-[var(--brand-muted)] mb-1">Usuario</label>
              ...
              <div>
                <label className="block text-[11px] font-semibold text-[var(--brand-muted)] mb-1">Contraseña</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-2.5 flex items-center pointer-events-none text-[var(--brand-accent)]">
                    <Lock size={14} />
                  </div>
            ...
            {miembrosEquipo.map((miembro, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 mt-2">
                  {miembro.linkedin && (
                    <a href={miembro.linkedin} target="_blank" rel="noopener noreferrer" className="text-[var(--brand-accent)] hover:opacity-80 transition-opacity" title="LinkedIn">
                  ...
                  {miembro.github && (
                    <a href={miembro.github} target="_blank" rel="noopener noreferrer" className="text-[var(--brand-accent)] hover:opacity-80 transition-opacity" title="GitHub">
                      <svg className="w-3.5 h-3.5 fill-current" viewBox="0 0 24 24">
